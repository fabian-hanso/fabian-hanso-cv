...
const SideStaggerNavigation = () => {
  ...
  return (
    <motion.nav
      ...
      className="fixed right-0 top-0 h-screen flex-col items-end justify-between py-4 pl-8 hidden md:flex"
    >
  ...
  if (title) {
    return (
      <a href="#">
        <motion.div
          ref={ref}
          className="group relative bg-gray-700 transition-colors hover:bg-fourth"
          ...
          <AnimatePresence>
            {isHovered && (
              <motion.span
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute left-0 top-0 z-10 w-full pt-2 font-bold uppercase text-gray-700 transition-colors group-hover:text-fourth"
              >
                {title}
              </motion.span>
            )}
